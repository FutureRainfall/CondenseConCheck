...
- **`Condensation.py`**：输入图 G，调用 `SCC.py`输出 G 的缩点图 C ，返回字典，其中包括键 'abj', 'mapping', 'members'。
  ...
  - **'mapping'** 为节点 id -> SCC 编号映射字典，格式为 'mapping': {node_id: SCC_id}，记录原图节点在哪个 SCC 中
  - **'members'** 为 SCC 编号 -> 节点 id 的映射字典，格式为 'members': {SCC_id: set(node_id)}，记录一个 SCC 中包含原图的哪些节点
- **`map_reader.py`**：读取 `100Sim-4.xlsx`地图并建立地图字典，**需要安装依赖**，运行 `pip install -r "requirements.txt"`
